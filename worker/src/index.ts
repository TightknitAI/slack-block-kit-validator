...
const corsPreflight = (): Response => new Response(null, { status: 204, headers: CORS_HEADERS });

const notFound = (env: Env): Response =>
  new Response(
    JSON.stringify({
      error: "not_found",
      message: "Unknown route. See / for the playground or /openapi.json for the API spec.",
      meta: { provider: env.PROVIDER_NAME, providerUrl: env.PROVIDER_URL, repo: env.REPO_URL },
    }),
  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (!rl.allowed) {
      ...
      return new Response(rl.response.body, {
        ...
        headers: { ...Object.fromEntries(rl.response.headers), ...CORS_HEADERS },
      });
    }
    const extraHeaders = { ...rl.headers, ...CORS_HEADERS };
